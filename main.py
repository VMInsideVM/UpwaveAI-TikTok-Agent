...
def data_filter(
        like_to_followers_ratio=[],
        engagement_rate=[],
        total_units_sold_in_last_28_days=[],
        units_sold_via_video_in_last_28_days=[],
        units_sold_via_stream_in_last_28_days=[],
        average_video_views_in_last_28_days=[],
        average_views_per_shoppable_video_in_last_28_days=[],
        creator_debut_time='all'
    ):
    """
    数据筛选过滤器函数
    
    Args:
        like_to_followers_ratio (list): 点赞与粉丝比例范围
            - 格式: [最小值, 最大值]
            - 例如: [1000, 10000] 表示点赞与粉丝比例在1000-10000之间
            - 空列表表示不筛选点赞与粉丝比例
        engagement_rate (list): 参与率范围
            - 格式: [最小值, 最大值]
            - 例如: [1000, 10000] 表示参与率在1000-10000之间
            - 空列表表示不筛选参与率
        total_units_sold_in_last_28_days (list): 过去28天总销量范围
            - 格式: [最小值, 最大值]
            - 例如: [1000, 10000] 表示过去28天总销量在1000-10000之间
            - 空列表表示不筛选过去28天总销量
        units_sold_via_video_in_last_28_days (list): 过去28天通过视频销量范围
            - 格式: [最小值, 最大值]
            - 例如: [1000, 10000] 表示过去28天通过视频销量在1000-10000之间
            - 空列表表示不筛选过去28天通过视频销量
        units_sold_via_stream_in_last_28_days (list): 过去28天通过直播销量范围
            - 格式: [最小值, 最大值]
            - 例如: [1000, 10000] 表示过去28天通过直播销量在1000-10000之间
            - 空列表表示不筛选过去28天通过直播销量
        average_video_views_in_last_28_days (list): 过去28天平均视频观看量范围
            - 格式: [最小值, 最大值]
            - 例如: [1000, 10000] 表示过去28天平均视频观看量在1000-10000之间
            - 空列表表示不筛选过去28天平均视频观看量
        average_views_per_shoppable_video_in_last_28_days (list): 过去28天平均可购物视频观看量范围
            - 格式: [最小值, 最大值]
            - 例如: [1000, 10000] 表示过去28天平均可购物视频观看量在1000-10000之间
            - 空列表表示不筛选过去28天平均可购物视频观看量
        creator_debut_time (): 首次视频发布时间 'all','7','28','90','180'
            - 'all': 全部时间
            - '7': 过去7天
            - '28': 过去28天
            - '90': 过去90天
    ...
    """
    # TODO: 实现数据过滤逻辑
    if like_to_followers_ratio == []:
        url_params0 = ''
    else:
        url_params0 = f"&like_followers_v1_rate={like_to_followers_ratio[0]},{like_to_followers_ratio[1]}"
    if engagement_rate == []:
        url_params1 = ''
    else:
        url_params1 = f"&interaction_v1_rate={engagement_rate[0]},{engagement_rate[1]}"
    if total_units_sold_in_last_28_days == []:
        url_params2 = ''
    else:
        url_params2 = f"&sale_28d_count={total_units_sold_in_last_28_days[0]},{total_units_sold_in_last_28_days[1]}"
    if units_sold_via_video_in_last_28_days == []:
        url_params3 = ''
    else:
        url_params3 = f"&prod_video_28d_count={units_sold_via_video_in_last_28_days[0]},{units_sold_via_video_in_last_28_days[1]}"
    if units_sold_via_stream_in_last_28_days == []:
        url_params4 = ''
    else:
        url_params4 = f"&prod_live_28d_count={units_sold_via_stream_in_last_28_days[0]},{units_sold_via_stream_in_last_28_days[1]}"
    if average_video_views_in_last_28_days == []:
        url_params5 = ''
    else:
        url_params5 = f"&avg_28d_play_count={average_video_views_in_last_28_days[0]},{average_video_views_in_last_28_days[1]}"
    if average_views_per_shoppable_video_in_last_28_days == []:
        url_params6 = ''
    else:
        url_params6 = f"&avg_28d_sale_play_count={average_views_per_shoppable_video_in_last_28_days[0]},{average_views_per_shoppable_video_in_last_28_days[1]}"
    if creator_debut_time == 'all':
        url_params7 = ''
    else:
        url_params7 = f"&first_video_time={creator_debut_time}"
    return f"{url_params0}{url_params1}{url_params2}{url_params3}{url_params4}{url_params5}{url_params6}{url_params7}"
...
def get_table_data(max_pages=None, filename=None, sheet_name="表格数据"):
    """
    循环获取多页表格数据并汇总导出到Excel文件

    Args:
        max_pages (int, optional): 最大页数，如果不指定则自动获取实际最大页码
        filename (str, optional): Excel文件名，如果不指定则使用时间戳生成
        sheet_name (str): Excel工作表名称，默认为"表格数据"

    Returns:
        str: 导出的Excel文件路径，如果失败则返回空字符串
    """
    try:
        ...
        for page_num in range(1, max_pages + 1):
            ...
            for row in page_data:
                row.append(f"第{page_num}页")  # 在每行末尾添加页面信息
        ...
        # 创建DataFrame
        df = pd.DataFrame(all_data, columns=headers + ['页面'])
        ...
        # 定义需要排除的列名
        columns_to_exclude = ['近28天销量趋势', '操作', '页面']
        ...
        if filename is None:
            ...
            filename = f"多页表格数据_{timestamp}.xlsx"
        ...
        print(f"多页表格数据已成功导出到: {filename}")
        print(f"共导出 {total_rows} 行数据，{len(columns_to_keep)} 列（已过滤），来自 {len(set([row[-1] for row in all_data]))} 页")
...
def build_complete_url(
    ...
    Args:
        ...
        like_to_followers_ratio (list): 点赞与粉丝比例范围，默认[]
        engagement_rate (list): 参与率范围，默认[]
        total_units_sold_in_last_28_days (list): 过去28天总销量范围，默认[]
        units_sold_via_video_in_last_28_days (list): 过去28天通过视频销量范围，默认[]
        units_sold_via_stream_in_last_28_days (list): 过去28天通过直播销量范围，默认[]
        average_video_views_in_last_28_days (list): 过去28天平均视频观看量范围，默认[]
        average_views_per_shoppable_video_in_last_28_days (list): 过去28天平均可购物视频观看量范围，默认[]
        creator_debut_time (str): 首次视频发布时间，默认'all'
        product_name (str): 商品类型名称，默认None
        main_category (str): 商品大类名称，默认None
        sort_param (str): 排序参数，默认None（不添加排序参数）
                          如果需要排序，请在生成的URL后面手动添加 get_sort_suffix() 的返回值
    ...
    # 6. 调用 data_filter 获取数据过滤后缀
    data_suffix = data_filter(
        like_to_followers_ratio,
        engagement_rate,
        total_units_sold_in_last_28_days,
        units_sold_via_video_in_last_28_days,
    ...
    if data_suffix:
        url_suffixes.append(data_suffix)

    # 7. 只有在显式传递 sort_param 时才添加排序后缀
    # 否则排序后缀应该在导出时根据用户选择的维度动态添加
    if sort_param is not None:
        sort_suffix = get_sort_suffix(sort_param)
        if sort_suffix:
            url_suffixes.append(sort_suffix)
