...
### 🚀 Multi-Tab Concurrent Scraping (2025-01 新增)

**达人详情爬取现已支持多标签页并发，大幅提升效率！**
...
|---------|-------------|---------|
| 串行（旧版） | 40-50 分钟 | 基准 |
| 3 并发标签页 | 15-18 分钟 | 2.5x ⚡ |
| 5 并发标签页 | 10-12 分钟 | 4x ⚡⚡ |

#### 使用方法

在调用 `/process_influencer_list` API 时，传入 `max_concurrent` 参数：

```python
# Python 示例
import requests

response = requests.post("http://127.0.0.1:8000/process_influencer_list", json={
    "json_file_path": "output/tiktok_达人推荐_口红_20251110_123456.json",
    "cache_days": 3,
    "max_concurrent": 5  # 并发标签页数量（1-10，默认3）
})
```

```bash
# curl 示例
curl -X POST "http://127.0.0.1:8000/process_influencer_list" \
...
- **并发控制**: 使用 `asyncio.Semaphore` 限制最大并发数
- **标签页管理**: 动态创建和关闭标签页（`context.new_page()` / `page.close()`）
- **反爬保护**: 随机延迟 0.5-1.5 秒，避免触发反爬机制
- **容错机制**: 单个失败不影响其他任务（`asyncio.gather` with `return_exceptions=True`）

#### 并发数建议

- **推荐**: 3-5 个并发（平衡效率和反爬风险）
- **保守**: 1-3 个（适合敏感场景）
- **激进**: 5-10 个（需要充足内存，约 500MB-1GB）

⚠️ **注意**: 并发数越高，反爬风险越大，建议不要超过 10 个。
